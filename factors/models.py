...
class LifeTable(object):
    ...
    def get_lx(self):
        ...
        return {gender: df.ix[select].ix[gender] for gender in ('M', 'F')}
    ...
    def cf_ay_avg(self, age_insured, sex_insured, pension_age=None, **kwargs):
        ...
        assert sex_insured in ('M', 'F'), "sex insured should be either M of F!"
        sex_beneficiary = 'F' if sex_insured == 'M' else 'M'
        delta = int(self.params['delta'])
        sign = 1 if sex_insured == 'M' else -1
        gamma3 = self.adjust[sex_beneficiary][insurance_type]['CX3']
        tbl_beneficiary = (self.lx['F']['lx'] if sex_insured == 'M'
                           else self.lx['M']['lx'])
    ...
    def create_lookup_table(self, intrest):
        ...
        s = pd.DataFrame({'gender': (UPAGE - LOWAGE) * ['M'] +
                         (UPAGE - LOWAGE) * ['F'],
